...
name = input('Vvedite imya ustroistva: ')
list_keys = list(london_co[name].keys())
str_keys = str(list_keys)
str_keys = str_keys.strip('[]')
par = input('Vvedite imya parametra (' + str_keys.replace("'","") + '): ' )
par_to_read = par.lower()
print(london_co[name].get(par_to_read, 'TAKOGO PARAMETRA NETU GAVNO IDI NAHUI'))
